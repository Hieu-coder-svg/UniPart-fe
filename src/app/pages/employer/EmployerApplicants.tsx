...
export default function EmployerApplicants() {
  ...
  const filteredApplicants = useMemo(() => {
    ...
    // Sắp xếp đơn mới nhất từ trên xuống dưới (theo appliedAt)
  ...
  const handleAccept = async (id: number) => {
    const result = await Swal.fire({
      title: 'Xác nhận',
      text: 'Bạn có chắc chắn muốn CHẤP NHẬN ứng viên này không? (Hệ thống sẽ tự động gửi email và thông báo cho ứng viên)',
      icon: 'question',
  ...
  const handleReject = async (id: number) => {
    const result = await Swal.fire({
      title: 'Xác nhận',
      text: 'Bạn có chắc chắn muốn TỪ CHỐI ứng viên này không?',
      icon: 'warning',
      showCancelButton: true,
  ...
  const handleComplete = async (id: number) => {
    const result = await Swal.fire({
      title: 'Xác nhận',
      text: 'Bạn có chắc chắn xác nhận ứng viên đã HOÀN THÀNH công việc này không? (Hệ thống sẽ gửi thông báo yêu cầu ứng viên đánh giá)',
      icon: 'question',
      showCancelButton: true,
  ...
  const handleViewProfile = async (studentId: number, studentName: string, studentEmail: string, studentPhone?: string, studentUniversity?: string, studentMajor?: string, studentAvatar?: string) => {
          ...
          <div className="flex gap-2 mt-6 border-b-2 border-gray-200 overflow-x-auto">
            {tabs.map((tab) => (
              <button
                ...
                className={`px-6 py-3 -mb-0.5 border-b-4 transition-all font-medium whitespace-nowrap ${activeTab === tab.key
                ...
                >
                  <div className="flex flex-col lg:flex-row gap-4 md:gap-6">
                    ...
                    <div className="flex flex-col gap-3 lg:min-w-[180px]">
                      ...
                      {/* View Profile button */}
                      <button
                        onClick={() => handleViewProfile(Number(applicant.studentId), applicant.studentName, applicant.studentEmail || "", applicant.studentPhone, applicant.studentUniversity, applicant.studentMajor, applicant.studentAvatar)}
              ...
              {getPageNumbers().map((pageVal, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`w-10 h-10 rounded-xl text-sm font-bold transition-all border-2 ${active
                        ...
                        {selectedStudent.rating !== undefined && selectedStudent.rating > 0 && (
                          <div className="flex items-center gap-1 mt-1">
                            {[1, 2, 3, 4, 5].map((star) => (
                              <Star
                                key={star}
                                className={`w-4 h-4 ${star <= Math.round(selectedStudent.rating!)
                    ...
                    <div>
                      ...
                      <div className="flex items-center justify-center gap-2">
                        {[1, 2, 3, 4, 5].map((star) => (
                          ...
                          >
                            <Star
                              className={`w-10 h-10 transition-all duration-200 ${star <= (reviewHover || reviewRating)
