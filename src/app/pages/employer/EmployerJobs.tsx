...
export default function EmployerJobs() {
  ...
  return (
    <div className="w-full max-w-full overflow-x-hidden p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto box-border">
      {/* Header */}
      ...
      {/* Package Info Banner */}
      {packageInfo && (
        <div className="bg-white border border-indigo-100 rounded-2xl p-3 md:p-5 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 md:gap-6">
          <div className="flex items-start gap-3">
            <div className="w-9 h-9 md:w-12 md:h-12 bg-indigo-50 rounded-xl flex items-center justify-center flex-shrink-0">
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-1.5 md:gap-3">
                <span className="px-2 py-0.5 bg-blue-50 text-blue-700 text-[10px] md:text-xs font-semibold rounded-md border border-blue-100 flex items-center gap-1">
                  <Package className="w-3 h-3" />
                  Tin thường (lẻ): {packageInfo.remainingPosts}
                </span>
                <span className="px-2 py-0.5 bg-orange-50 text-orange-700 text-[10px] md:text-xs font-semibold rounded-md border border-orange-100 flex items-center gap-1">
                  <Zap className="w-3 h-3" />
                  Tin gấp (lẻ): {packageInfo.remainingUrgentPosts}
                </span>
                <span className="px-2 py-0.5 bg-purple-50 text-purple-700 text-[10px] md:text-xs font-semibold rounded-md border border-purple-100 flex items-center gap-1">
                  <Package className="w-3 h-3" />
                  Tin thường (gói tháng): {packageInfo.remainingMonthlyPosts}
                </span>
                <span className="px-2 py-0.5 bg-pink-50 text-pink-700 text-[10px] md:text-xs font-semibold rounded-md border border-pink-100 flex items-center gap-1">
                  <Zap className="w-3 h-3" />
          ...
          <Link
            ...
            className="px-4 py-2 bg-white border border-gray-200 text-gray-700 rounded-lg font-medium hover:bg-gray-50 transition-colors whitespace-nowrap shadow-sm text-sm"
          >
            Nâng cấp gói
          </Link>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        {stats.map((stat, idx) => (
          <div key={idx} className="bg-white rounded-xl p-3 md:p-5 border border-gray-100 shadow-sm flex items-center gap-2 md:gap-4">
            <div className={`w-9 h-9 md:w-12 md:h-12 rounded-full flex items-center justify-center flex-shrink-0 ${stat.color}`}>
              <stat.icon className="w-4 h-4 md:w-6 md:h-6" />
            </div>
            <div>
              <div className="text-xl md:text-2xl font-bold text-gray-900">{stat.value}</div>
              <div className="text-[10px] md:text-sm font-medium text-gray-500 leading-tight">{stat.label}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-3">
        {paginatedJobs.length === 0 ? (
          ...
          paginatedJobs.map((job) => {
            ...
            return (
            <div key={job.id} className="bg-white rounded-xl border border-gray-100 p-3 sm:p-5 shadow-sm hover:border-orange-200 transition-colors">
              <div className="flex flex-col lg:flex-row justify-between gap-4 lg:gap-6">
                <div className="flex gap-3 sm:gap-4 items-start">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 bg-orange-50 text-orange-600 rounded-lg flex items-center justify-center text-base sm:text-lg font-bold border border-orange-100 shrink-0">
                  ...
                  <div>
                    ...
                    <p className="text-xs sm:text-sm text-gray-500 mb-2 sm:mb-3">{job.employerName}</p>
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center lg:items-end gap-4 lg:gap-6 border-t lg:border-t-0 border-gray-100 pt-3 sm:pt-4 lg:pt-0">
                  <div className="flex w-full sm:w-auto justify-between sm:justify-start gap-4 sm:gap-6 text-sm">
                    <div className="text-center">
                      <div className="text-base sm:text-lg font-semibold text-gray-900">{job.viewCount || 0}</div>
                      <div className="text-[10px] sm:text-xs text-gray-500">Lượt xem</div>
                    </div>
                    <div className="text-center">
                      <div className="text-base sm:text-lg font-semibold text-gray-900">{applicantsCount}</div>
                      <div className="text-[10px] sm:text-xs text-gray-500">Ứng viên</div>
                    </div>
                    <div className="text-center">
                      <div className="text-base sm:text-lg font-semibold text-orange-600">{job.vacancies ?? 0}</div>
                      <div className="text-[10px] sm:text-xs text-gray-500">Còn chỗ</div>
                    </div>
                  </div>
